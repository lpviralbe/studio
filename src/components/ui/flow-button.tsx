...
export function FlowButton({ text = "Modern Button", className, ...props }: FlowButtonProps) {
  return (
    ...
    >
      ...
      {/* Right arrow (arr-1) */}
      <ArrowRight
        className="absolute w-5 h-5 right-4 stroke-foreground fill-none z-[9] group-hover:right-[-25%] group-hover:stroke-primary-foreground transition-all duration-[800ms] ease-[cubic-bezier(0.34,1.56,0.64,1)]"
      />
    </button>
  );
}
